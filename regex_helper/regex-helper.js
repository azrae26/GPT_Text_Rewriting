/**
 * regex_helper/regex-helper.js - 正則表達式輔助模組
 * 功能：提供智能正則表達式創建和文本匹配功能
 * 職責：
 * - 智能模式識別：自動識別正則表達式模式和普通文本
 * - 正則表達式創建：支援從文本和模式創建正則表達式
 * - 特殊字符處理：正確轉義和處理特殊字符（如 *, **, / 等）
 * - 錯誤處理：提供安全的錯誤處理和失敗回退機制
 * - 多格式支援：支援 /pattern/flags 格式和原始模式格式
 * 
 * 使用場景：
 * - 文本替換功能的正則表達式生成
 * - 高亮功能的文本匹配模式創建
