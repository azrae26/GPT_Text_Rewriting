...
- **`default.js`**：所有腳本的第一個依賴。提供 `LogUtils`（格式 `[FileName][HH:MM:SS]`）、預設設定值、`window.SharedUrlWatcher`（共用 URL 變化監聽，單一真相）。直接宣告為 `const LogUtils`，不用 `window.LogUtils`。**慣例：需偵測 SPA URL 變化的模組一律 `SharedUrlWatcher.subscribe(cb)`，禁止再各自 `new MutationObserver` 輪詢 `location.href`（每次 DOM 變動都觸發、且易洩漏）**
