...
- **`default.js`**：所有腳本的第一個依賴。提供 `LogUtils`（格式 `[FileName][HH:MM:SS]`）、預設設定值、`window.SharedUrlWatcher`（共用 URL 變化監聽，單一真相）、`window.SharedTypingScheduler`（打字防抖排程，單一真相）。直接宣告為 `const LogUtils`，不用 `window.LogUtils`。**慣例：需偵測 SPA URL 變化的模組一律 `SharedUrlWatcher.subscribe(cb)`，禁止再各自 `new MutationObserver` 輪詢 `location.href`（每次 DOM 變動都觸發、且易洩漏）**。**慣例：需在打字時更新 overlay（高亮／替換預覽／diff 泡泡／股票）的模組一律用 `SharedTypingScheduler.create(fn)` 綁 input/compositionend 排程（單一真相）。預設即時模式（rAF 合流、與打字同步、零延遲）——使用者要即時，延遲即 lag。即時要夠快靠各模組做成「增量更新」（快取靜態樣式、不每字全清重建、前綴復用；highlight 另有**視窗化**：打字只算可見視窗內、畫面外停手後 `requestIdleCallback` 補回），詳與**坑**見 slowdown-leak-debug skill §7。⚠ 按住鍵仍有「宿主每字重算」地板：插件每字碰任一 DOM（overlay／測量鏡像 div）即觸發 uanalyze 重排自身 MUI，與 overlay 數無關、消不掉（故只 highlight 視窗化有用、diff/manual-replace/stock 視窗化試過無增益已回退；要即時就接受地板，詳 §7）。另有防抖模式（`SharedTypingScheduler.enabled`，預設關，僅備援/A-B）。捲動跟隨只改 transform，不走此排程**
...
**本文件必須與程式碼保持同步。** 以下情況發生時，AI 必須主動更新本文件（以及同步更新 `AGENTS.md`）：
